#!/usr/bin/env python3
"""Audit exact v17.0 plus eye textures and the permitted iris-stack Z-depth correction."""
...
def all_position_users(doc):
 ...
 return u
def main():
 a=args();h=runpy.run_path(HELPER,run_name='__audit_v170_iris_lift_helpers__');od,ob,_=h['parse_glb'](a.original);pd,pb,_=h['parse_glb'](a.patched)
 ...
 for ai in sorted(users):
  ...
  if spread>1e-7:raise RuntimeError(f'non-uniform eye Z shift accessor {ai}: spread={spread}')
  avg=sum(dz)/len(dz)
  if avg<=0.0005 or avg>=0.005:raise RuntimeError(f'eye depth correction outside expected range accessor {ai}: {avg}')
  changed.append({'accessor':ai,'delta_z':avg,'vertices':len(dz),'users':users[ai]})
 ...
 print('V170_IRIS_LIFT_AUDIT',json.dumps({'revision':'v17.0-eye-texture-plus-iris-depth-fix','changed_position_accessors':changed,'changed_accessor_count':len(changed),'unchanged_position_accessor_count':unchanged,'x_y_unchanged':True,'eye_detail_shape_unchanged':True,'non_eye_geometry_unchanged':True,'head_geometry_unchanged':True,'sclera_geometry_unchanged':True,'node_hierarchy_and_transforms_unchanged':True,'topology_unchanged':True,'eye_textures':names},sort_keys=True))
