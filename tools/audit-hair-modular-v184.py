"""Post-export face-lock and mobile-quality audit for modular anime hair v18.4."""
...
REQUIRED = {
    "HairPremiumV184_Crown",
    "HairPremiumV184_CurtainBangs",
    "HairPremiumV184_HighPonytail",
    "HairPremiumV184_TailStrands",
}
...
def snapshot(path):
    ...
    old = sorted(
        ...
        if obj.type == "MESH" and (
            ...
            or obj.name.startswith("HairPremiumV183")
...
def main():
    ...
    if any(row["parent"] != "BL_HAIR_ASSET" for row in hair.values()):
        raise RuntimeError(f"HAIR: v18.4 objects not under BL_HAIR_ASSET: {hair}")
    total = sum(row["triangles"] for row in hair.values())
    if not (22000 <= total <= 90000):
        raise RuntimeError(f"HAIR: v18.4 total outside mobile-quality gate: {total}")
    if hair["HairPremiumV184_Crown"]["triangles"] < 6000:
        raise RuntimeError("HAIR: Blender-chan crown lost too much density")
    if hair["HairPremiumV184_CurtainBangs"]["triangles"] < 3000:
        raise RuntimeError("HAIR: Curtain Bangs smoothing insufficient")
    if hair["HairPremiumV184_HighPonytail"]["triangles"] < 2500:
        raise RuntimeError("HAIR: High Ponytail smoothing insufficient")
    if hair["HairPremiumV184_TailStrands"]["triangles"] < 800:
        raise RuntimeError("HAIR: tail strand breakup missing")
    if old:
        raise RuntimeError("HAIR: previous hairstyle survived replacement: " + ", ".join(old[:30]))

    vulnerable = sorted(name for name in before if "lash" in name.lower() or "canthus" in name.lower())
    if not vulnerable:
        raise RuntimeError("FACE LOCK: lash/canthus helpers missing in baseline")
    ...
    print("HAIR_MODULAR_V184_AUDIT", json.dumps(summary, sort_keys=True))
