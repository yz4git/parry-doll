"""Post-export structural audit for fresh David Onizaki heroine v17.3."""
...
def find_head():
    ...
    if exact and exact.type == "MESH":
        return exact
    candidates = [o for o in bpy.data.objects if o.type == "MESH" and under(o, "BL_HEAD_ASSET") and "Head" in o.name]
    if not candidates:
        raise RuntimeError("v17.3 donor head not found after GLB re-import")
...
def main():
    ...
    with open(a.config, "r", encoding="utf-8") as f:
        cfg = json.load(f)
    if cfg.get("revision") != "v17.3":
        raise RuntimeError("unexpected v17.3 audit config")
    ...
    if len(new_eye) < cfg["audit"]["min_new_eye_parts"]:
        raise RuntimeError(f"v17.3 eye count below target: {len(new_eye)}")
    ...
    if len(lips) < cfg["audit"]["min_new_lip_parts"]:
        raise RuntimeError(f"v17.3 lip count below target: {len(lips)}")
    ...
    for n in ("BL_EYELID_L", "BL_EYELID_R", "BeautyMarkV173"):
        if bpy.data.objects.get(n) is None:
            raise RuntimeError("missing portrait runtime node: " + n)
    ...
    print("FRESH_DONOR_V173_AUDIT", json.dumps({
        "revision": "v17.3",
