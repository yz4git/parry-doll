...
def cached_pixels(img):
    w, h = int(img.size[0]), int(img.size[1])
    px = array('f', [0.0]) * (w * h * 4)
    img.pixels.foreach_get(px)
    return w, h, px


def sample_cached(cache, uv):
    w, h, px = cache
...
def blue_score(rgb):
    r, g, b = rgb
    lum = (r + g + b) / 3.0
    if lum < 0.075 or lum > 0.78 or b < 0.24:
        return -1.0
...
def donor_blue_surface(hair, donor_mat):
    ...
    for poly in hair.data.polygons:
        ...
        for j in range(1, len(vids) - 1):
            ...
            sc = blue_score(sample_cached(cache, center))
            if sc < 0.0:
                rejected += 1
                continue
...
def main():
    ...
    report = {"revision":"v18.5-r14","scope":"seven-crown-lock-blue-atlas-exact-donor","geometry_unchanged":True,"face_unchanged":True,"main_hair_unchanged":True,"bangs_unchanged":True,"side_hair_unchanged":True,"ponytail_unchanged":True,"scalp_gap_material_unchanged":True,"crown_lock_count":len(locks),"exact_shared_material":donor_mat.name,"donor_texture":donor_img,"donor_uv_layer":donor_uv,"transferred_uv_layer":UV_NAME,"eligible_blue_triangles":eligible,"rejected_nonblue_triangles":rejected,"blue_score_min":score_min,"blue_score_max":score_max,"prior_lock_materials":prior_materials,"per_lock_transfer":per_lock,"output_bytes":out.stat().st_size}
