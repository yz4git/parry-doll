"""Post-export face lock and quality audit for Blender-chan hair v18.2."""
...
def snapshot(path):
    ...
    hair = {}
    for obj in bpy.data.objects:
        if obj.type == "MESH" and obj.name.startswith(FINAL_PREFIX):
            hair[obj.name] = {
    ...
    old = sorted(
        ...
        if obj.type == "MESH" and (
            ...
            or (under(obj, "BL_HAIR_ASSET") and hair_name(obj) and not obj.name.startswith(FINAL_PREFIX))
        )
    )
...
def main():
    ...
    required = {"HairPremiumV182_Main", "HairPremiumV182_Bun", "HairPremiumV182_Strands"}
    ...
    if total < 70000:
        raise RuntimeError(f"HAIR: insufficient evaluated detail: {total} tris")
    if total > 125000:
        raise RuntimeError(f"HAIR: mobile budget exceeded: {total} tris")
    if old:
        raise RuntimeError("HAIR: previous hairstyle mesh survived replacement: " + ", ".join(old[:30]))

    # These were previously at risk because they live in/near the hair hierarchy.  If present in the
    # accepted base, they must survive byte-for-geometry unchanged.
    vulnerable = sorted(name for name in before if "lash" in name.lower() or "canthus" in name.lower())
    if not vulnerable:
        raise RuntimeError("FACE LOCK: expected lash/canthus helpers were not found in accepted base")
    ...
    print("HAIR_BLENDERCHAN_V182_AUDIT", json.dumps(summary, sort_keys=True))
