#!/usr/bin/env python3
"""Apply the only permitted v17.0 geometry fix: lift the existing iris stack out of the sclera.

The restored model stays exact v17.0 except for Z depth on the already-existing
IrisOuter/IrisInner/IrisRays/Pupil/EyeLight planes. X/Y, head/body/sclera geometry, topology,
indices, hierarchy and node transforms remain untouched. This resolves only the known burial
inside the original sclera globe; it does not move the eye stack to the donor forehead/face.
"""
...
CLEARANCE={'IrisOuterV167':0.00018,'IrisInnerV167':0.00030,'IrisRaysV167':0.00040,'PupilV167':0.00052,'EyeLightV167A':0.00064,'EyeLightV167B':0.00062}
def args():
...
def layout(doc,ai):
 ...
 bv=doc['bufferViews'][acc['bufferView']];stride=int(bv.get('byteStride',12));base=int(bv.get('byteOffset',0))+int(acc.get('byteOffset',0));return acc,base,stride
...
def main():
 ...
 for n in doc.get('nodes',[]):
  ...
  side=side_of(name);mi=n['mesh'];prims=doc['meshes'][mi]['primitives'];vals=positions(doc,binary,prims[0]['attributes']['POSITION']);t=n.get('translation',[0,0,0]);s=n.get('scale',[1,1,1])
 ...
 for n in doc.get('nodes',[]):
  ...
  for p in doc['meshes'][mi].get('primitives',[]):
   ai=p.get('attributes',{}).get('POSITION')
 ...
 doc.setdefault('asset',{}).setdefault('extras',{}).update({'parryDollRevision':'v17.0-eye-texture-plus-iris-depth-fix','parryDollGeometryPolicy':'only-existing-iris-pupil-highlight-POSITION-z-changed','parryDollScleraGeometryChanged':False,'parryDollFaceGeometryChanged':False})
 h['write_glb'](a.output,doc,bytes(binary))
 out={'revision':'v17.0-eye-texture-plus-iris-depth-fix','sclera_front_z':{str(k):v for k,v in sclera.items()},'clearance':CLEARANCE,'changed_nodes':sorted(nodes),'changed_position_accessors':{str(k):v for k,v in sorted(changed.items())},'changed_geometry_scope':'existing iris/pupil/highlight rigid Z-depth only','x_y_changed':False,'topology_changed':False,'node_transforms_changed':False,'sclera_geometry_changed':False,'face_geometry_changed':False,'output_bytes':os.path.getsize(a.output)}
