...
LIP=material('Lip',(0.335,0.125,0.145),0,.39)
...
def add_reference_head_v120(p,name,mat,segments=112):
 ...
 nose_proj=profile_ctrl['noseProjection']*.735;nose_width=profile_ctrl['noseWidth']
 ...
 base_profile=[
  (.090,.1002),(.060,.1010),(.038,.1000),(.020,.0976),(.006,.0988),(-.008,.1048),
 ...
 for yy,w,back,front,zoff in sections:
  ...
  for i in range(segments):
   ...
   if sp>0:
    ...
    pw=(.0275*nose_width) if yy>-.068 else (.043 if yy>-.108 else .039)
   ...
   if yy<-.112:
    ...
    render_y+=.0100*under*(1.0-frontness**1.55)
    ...
    render_y+=.0315*under*(backness**1.10)
...
def add_reference_head_v140(p,name,mat,segments=128):
  ...
  for i in range(segments):
   ...
   if sp>0:
    ...
    for side in (-1,1):
     ...
     z-=fm*.00105*crease
     sidewall=math.exp(-((x-side*.0108)/.0132)**2-((yy+.018)/.0400)**2)
     z+=fm*.00155*sidewall
   ...
   if yy<-.110:
    ...
    render_y+=.0225*under*sideback+.0460*under*(backness**1.04)
 ...
 bpy.context.view_layer.objects.active=o;bpy.ops.object.modifier_apply(modifier=sub.name)
 o['face_rebuild']='v14.7';o['topology']='multiview_quad_cage_with_true_orbital_openings';o['reference']='supplied_profile_plus_existing_front_audits'
 return parent(o,p)
...
# REFERENCE_V138: supplied-reference portrait pass softens the nose-tip S-curve, gives the lips a cleaner profile break and strengthens the exact-profile iris/lash silhouette without changing frontal face width.
# REFERENCE_V139: eye-reveal fringe pass lifts the thick brow-level bang volumes away from the profile eye and replaces their lower continuation with fine face-framing strands, preserving crown coverage and the accepted v13.8 face.
...
# REFERENCE_V147: continuous-profile silhouette pass rebuilds the centre-line forehead/nasal-root/nose/lip/chin S-curve from the supplied side portrait while leaving frontal widths, eye spacing, jaw width and the accepted neck/hair systems unchanged.
# REFERENCE_V148: exact-profile eye-readability pass enlarges only the YZ side-facing sclera/iris/pupil aperture and moves the profile lids/lashes slightly outward, matching the supplied portrait without changing frontal eye spacing or width.
...
# REFERENCE_V153: nasion/bridge profile pass deepens the orbital-to-nose root break and builds a narrow continuous dorsum into the accepted small nose tip without changing the mouth, jaw or frontal face width.
# REFERENCE_V154: exact-profile eye hero pass enlarges and slightly externalizes only the side-facing sclera/iris/pupil aperture, matching lids and lashes to that silhouette while preserving the accepted frontal/three-quarter almond eye.
# REFERENCE_V160: v14.0 root face rebuild replaces the accumulated v13 centre-profile/RBF shell with one new multiview landmark cage: recessed orbit, continuous narrow nose, embedded lips, soft chin and rising under-jaw while preserving BL_HEAD, neck and blink runtime contracts.
# REFERENCE_V161: v14.1 base-cage correction strengthens the nasion-to-tip S profile, raises the rear under-jaw and seats larger exact-profile eyes deeper inside the rebuilt sockets; no return to v13 local silhouette patches.
# REFERENCE_V162: v14.2 compacts the lower-face cage, preserves a softer adult jaw instead of a long V, and places a true side-facing ocular layer on the lateral head surface so profile audits read an eye rather than a dot.
# REFERENCE_V163: v14.3 removes the detached lateral profile-eye plate after 3/4 audit exposed a duplicate-eye artifact; profile readability now comes from the original eye canthus wrapping farther around the orbital rim.
# REFERENCE_V164: v14.4 seats the real eye deeper, turns the side helper into a restrained canthus glint, and strengthens orbital, nasal, malar and mouth planes for a less mannequin-like close portrait.
# REFERENCE_V165: v14.5 rebuilds portrait readability around a larger but deeper-set almond aperture, clearer alar/nostril anatomy and fuller integrated lips while keeping the compact v14 jaw and single wrapped profile eye.
# REFERENCE_V166: v14.6 cuts true orbital apertures into the v14 head cage and replaces flat/profile eye helper plates with one embedded spherical sclera and shallow curved iris stack, so front, 3/4 and profile share the same physical eye.
# REFERENCE_V167: v14.7 targets the actual HeadShellV140 face loop for the orbital openings, wraps the outer canthus aperture around the side plane, and restores visible iris/pupil discs on the physical sclera surface.
...
add_reference_head_v140(HEAD,'HeadShellV140',SKIN,128)
_neck_r=W('neck')*.292
...
for side in(-1,1):
 ...
 add_strand(HEAD,f'EarringDropV137_{side}',[(side*.1460,-.043,-.010),(side*.1462,-.057,-.009),(side*.1460,-.071,-.010)],.00032,SILVER)
 add_box(HEAD,f'EarringTipV137_{side}',(side*.1460,-.075,-.010),(.0045,.010,.0045),SILVER,.0015)
 add_sphere(HEAD,f'EarConchaBowlV144_{side}',(side*.1451,-.020,-.016),(.0023,.0128,.0064),EAR_SHADOW,22,14)
...
face_front=.0974
eye_y=FACE140['eye_target']['center_y']
eye_x=FACE140['feature_landmarks']['eye_center_x']*ASSEMBLY120['head']['eyeSpacing']
eye_rx=FACE140['eye_target']['aperture_rx']*ASSEMBLY120['head']['eyeSize']
eye_ry=FACE140['eye_target']['aperture_ry']*ASSEMBLY120['head']['eyeSize']
eye_tilt=FACE140['eye_target']['tilt']
iris_scale=ASSEMBLY120['head']['irisScale']*FACE140['eye_target']['iris_scale_multiplier']
eye_contrast=ASSEMBLY120['head']['eyeContrast']*1.08
for side in(-1,1):
 ex=side*eye_x
 add_sphere(HEAD,f'EyeScleraGlobeV166_{side}',(ex,eye_y,.0832),(eye_rx*.620,eye_rx*.620,eye_rx*.620),SCLERA,48,32)
 # v14.6 removed: sclera/profile helper replaced by shared embedded globe
 # v14.6 removed: sclera/profile helper replaced by shared embedded globe
 # v14.6 removed: sclera/profile helper replaced by shared embedded globe
 # v14.6 removed: sclera/profile helper replaced by shared embedded globe
 # v14.6 removed: sclera/profile helper replaced by shared embedded globe
 ...
 add_strand(HEAD,f'UpperLashV119_{side}',[(inner,eye_y-eye_tilt+.0002,.10310),(ex-side*.0030,eye_y+.01055,.10378),(outer+side*.0012,eye_y+eye_tilt+.00045,.10322)],.00116*eye_contrast,HAIR)
 add_strand(HEAD,f'OuterLashV119_{side}',[(outer-side*.0036,eye_y+eye_tilt+.0017,.10325),(outer+side*.0058,eye_y+eye_tilt+.0052,.10330),(outer+side*.0108,eye_y+eye_tilt+.0037,.10318)],.00066*eye_contrast,HAIR)
 ...
 add_strand(HEAD,f'ProfileUpperLidV140_{side}',[(ex+side*eye_rx*1.075,eye_y+eye_ry*.66,.10015),(ex+side*eye_rx*1.098,eye_y+eye_ry*.30,.10655),(ex+side*eye_rx*1.105,eye_y-.0001,.10835)],.00029*eye_contrast,FACE_DARK)
 add_strand(HEAD,f'ProfileLowerLidV140_{side}',[(ex+side*eye_rx*1.075,eye_y-eye_ry*.61,.10030),(ex+side*eye_rx*1.098,eye_y-eye_ry*.28,.10630),(ex+side*eye_rx*1.105,eye_y-.0001,.10805)],.00018*eye_contrast,EYE_WET)
 add_strand(HEAD,f'WrappedCanthusLashV143_{side}',[(ex+side*eye_rx*1.04,eye_y+eye_tilt+.0030,.1033),(ex+side*eye_rx*1.11,eye_y+eye_tilt+.0050,.1068),(ex+side*eye_rx*1.18,eye_y+eye_tilt+.0040,.1100)],.00026*eye_contrast,HAIR)
 add_strand(HEAD,f'ProfileLashFanV138_A_{side}',[(outer-side*.0008,eye_y+eye_tilt+.0035,.10435),(outer+side*.0018,eye_y+eye_tilt+.0072,.1072),(outer+side*.0042,eye_y+eye_tilt+.0082,.1123)],.00036*eye_contrast,HAIR)
 add_strand(HEAD,f'ProfileLashFanV138_B_{side}',[(outer-side*.0005,eye_y+eye_tilt+.0026,.10430),(outer+side*.0020,eye_y+eye_tilt+.0038,.1078),(outer+side*.0047,eye_y+eye_tilt+.0028,.1118)],.00031*eye_contrast,HAIR)
 add_strand(HEAD,f'LowerLidV119_{side}',[(inner+side*.0040,eye_y-eye_tilt-.00035,.10292),(ex,eye_y-.00770,.10318),(outer-side*.0024,eye_y+eye_tilt-.00015,.10302)],.000215*eye_contrast,FACE_DARK)
 add_strand(HEAD,f'UpperLidFoldV119_{side}',[(inner+side*.0050,eye_y-eye_tilt+.0038,.10340),(ex,eye_y+.0137,.10375),(outer-side*.0060,eye_y+eye_tilt+.0036,.10342)],.00017,FACE_DARK)
...
# v13.14 reference-style layered crown and temple grouping.
_crown_layers_v144=[
...
# v13.9 reference-like eye-reveal fringe continuations.
for _side in (-1,1):
...
add_strand(HEAD,'ProfileWispV131_L',[(-.076,.122,.106),(-.084,.086,.102),(-.082,.052,.092),(-.070,-.002,.089),(-.058,-.060,.094)],.000032,HAIR)
add_strand(HEAD,'ProfileWispV131_R',[(.078,.126,.104),(.087,.090,.103),(.086,.054,.096),(.075,.008,.093),(.064,-.032,.095)],.000034,HAIR_HI)

# v13.12 fine sideburns leave the ear itself visible while preserving the supplied-reference softness.
for _side in (-1,1):
...
# v13.11 supplied-reference swept temple flow around the exposed ear.
for _side in (-1,1):
...
# v13.8 reference eye/ear framing wisps.
for _side in (-1,1):
