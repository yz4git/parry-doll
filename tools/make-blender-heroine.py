...
# Layered split panels preserve the long-leg read while matching the reference skirt envelope.
...
for side in(-1,1):
 for i in range(5):
  add_strand(HEAD,f'SideLock_{side}_{i}',[(side*(head_w*.35+i*.010),.120,head_d*.16),(side*(head_w*.47+i*.012),-.04,head_d*.28),(side*(head_w*.50+i*.012),-.33,head_d*.12),(side*(head_w*.43+i*.010),-.55,-.015)],.009,HAIR_HI if i==0 else HAIR)
PONY=empty('BL_PONY_DYNAMIC',HEAD)
for i in range(27):
 lane=(i-13)/13;spread=.030+.035*abs(lane)
 add_strand(PONY,f'Pony_{i}',[(lane*.020,.145,-head_d*.46),(lane*.065,.015,-head_d*.70),(lane*.115,-.37,-.38),(lane*.19,-.88,-.31),(lane*.28,-1.48,-.12)],.0095+(i%5)*.0011,HAIR_HI if i%7==0 else HAIR)
add_box(HEAD,'HairTie',(0,.135,-head_d*.47),(.105,.036,.042),SILVER,.010)
