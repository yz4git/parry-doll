...
const reviewCoreAnnounce=announce;announce=function(t,d){reviewCoreAnnounce(t==='体 勢 崩 し — 斬 れ'?'斬 れ':t,d)};
setCamera=function(){
 ...
 else if(distance>.6)cameraRig.yaw+=clamp(angleDelta(desired,cameraRig.yaw),-(cine>0?2.8:2.1)*feel.dt,(cine>0?2.8:2.1)*feel.dt);
 ...
 let back=(portrait?5.5:4.35)+Math.max(0,distance-4)*.18-(portrait?.28:.62)*cine,shoulder=(portrait?.65:1.9)+(portrait?.05:.16)*cine;
...
resolveSwing=function(){
 ...
 if(move&&move.finisher&&boss.hp<before){reviewCine=1.05;hitstop=Math.max(hitstop,.16);shake=Math.max(shake,.42);}
};
