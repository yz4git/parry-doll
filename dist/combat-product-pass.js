...
drawDoll=function(d){
 ...
 if(parryOutline){
  const alpha=clamp(p5ParryGlow/.20,0,1),col=`rgba(255,211,107,${(.22+.42*alpha).toFixed(3)})`;
  ...
  const hand=d.nodes.find(n=>n.name==='hand')||d.nodes[1];orb(hand.p,.14*d.spec.scale,'#ffd36b');
 }
};
