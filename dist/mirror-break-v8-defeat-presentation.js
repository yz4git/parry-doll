...
(()=>{
 ...
 const ko={pending:false,level:-1,t:0,quietT:0,releasedLevel:-1,opened:0,lastMotion:0,lastChestY:0,lastOpenT:0};
 window.__mirrorBreakV8DefeatState=ko;
 const MIN_SHOW=1.85,QUIET_NEED=.30,MAX_SHOW=3.20;
 ...
 function beginPresentation(){
  ...
  // v1 normally opens the reward on the first dead-boss frame. Reserve this level while the KO plays.
  state.choiceFor=level;transition=Math.max(transition,999);attackQueued=parryQueued=false;attackBuffer=parryBuffer=0;
 }
 function openReward(){
  const finishedLevel=ko.level,shownAt=ko.t;resetPending();state.choiceFor=-1;transition=Math.max(transition,999);
  if(typeof showCoreChoice==='function')showCoreChoice();
  if(state.choice){ko.releasedLevel=finishedLevel;ko.opened++;ko.lastOpenT=shownAt}
 }
 const baseReset=reset;reset=function(l=0){const out=baseReset(l);resetPending();ko.releasedLevel=-1;ko.lastOpenT=0;return out};
