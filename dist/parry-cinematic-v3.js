...
(()=>{
 ...
 function draw(){
  ...
  if(flash>0){c.save();c.globalCompositeOperation='screen';c.globalAlpha=(beat.perfect?.13:.10)*flash;c.fillStyle='#ffd36b';c.fillRect(0,0,W,H);c.restore()}
  const coreR=(beat.perfect?76:62)*(1+t*.48);c.save();c.globalCompositeOperation='lighter';const g=c.createRadialGradient(cx,cy,0,cx,cy,coreR);g.addColorStop(0,`rgba(255,255,255,${.62*fade})`);g.addColorStop(.09,`rgba(255,211,107,${.48*fade})`);g.addColorStop(.34,`rgba(255,184,74,${.20*fade})`);g.addColorStop(.70,`rgba(255,240,176,${.08*fade})`);g.addColorStop(1,'rgba(0,0,0,0)');c.fillStyle=g;c.fillRect(cx-coreR,cy-coreR,coreR*2,coreR*2);c.restore();
  for(const s of beat.streaks){const cs=Math.cos(s.a),sn=Math.sin(s.a),px=cx-sn*s.offset,py=cy+cs*s.offset,l=s.len*(.45+.55*Math.sin(Math.min(1,t*1.9)*Math.PI*.72));glowLine(px-cs*l*.52,py-sn*l*.52,px+cs*l*.48,py+sn*l*.48,'#ffffff',s.w,fade*.46,s.w*3.4);glowLine(px-cs*l*.46,py-sn*l*.46,px+cs*l*.42,py+sn*l*.42,beat.perfect?'#ffd36b':'#ffb84a',s.w*1.65,fade*.18,s.w*4.6)}
  for(const r of beat.rays){if(t<r.delay)continue;const q=clamp01((t-r.delay)/(1-r.delay)),grow=Math.sin(Math.min(1,q)*Math.PI*.76),L=r.len*grow,cs=Math.cos(r.a),sn=Math.sin(r.a),a=fade*(1-q*.52);glowLine(cx+cs*7,cy+sn*7,cx+cs*L,cy+sn*L,r.warm?'#ffd36b':'#ffb84a',r.w,a,r.w*3.2)}
  // One clear shock ring plus a faint delayed echo replaces the previous white-out.
  for(let k=0;k<2;k++){const q=clamp01(t-k*.07);if(q<=0)continue;const r=(22+q*(beat.perfect?132:104))*(1+k*.13);c.save();c.globalCompositeOperation='lighter';c.globalAlpha=fade*(.46-k*.22);c.strokeStyle=k?'#ffb84a':'#ffd36b';c.lineWidth=Math.max(1,3.3*(1-q));c.shadowColor=c.strokeStyle;c.shadowBlur=12;c.beginPath();c.ellipse(cx,cy,r,r*.43,-.16,0,Math.PI*2);c.stroke();c.restore()}
  c.save();c.globalCompositeOperation='lighter';for(const s of beat.sparks){const q=Math.min(1,t/(s.life/beat.max)),dist=s.sp*t,ang=s.a+s.spin*t*.035,x=cx+Math.cos(ang)*dist,y=cy+Math.sin(ang)*dist+t*t*52;c.globalAlpha=fade*(1-q*.48);c.fillStyle=Math.random()>.38?'#fff0b0':'#ffd36b';c.fillRect(x,y,s.size*(1-q*.60),s.size*(1-q*.60))}c.restore();
 }
