...
(()=>{
 ...
 function draw(){
  ...
  for(const r of beat.rays){if(t<r.delay)continue;const q=clamp01((t-r.delay)/(1-r.delay)),grow=Math.sin(Math.min(1,q)*Math.PI*.76),L=r.len*grow,cs=Math.cos(r.a),sn=Math.sin(r.a),a=fade*(1-q*.52);glowLine(cx+cs*7,cy+sn*7,cx+cs*L,cy+sn*L,r.warm?'#ff5260':'#d8fbff',r.w,a,r.w*3.2)}
