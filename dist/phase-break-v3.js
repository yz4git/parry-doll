...
(()=>{
 ...
 const style=document.createElement('style');style.textContent=`#pbMirror{position:fixed;left:50%;top:29%;transform:translate(-50%,6px) scale(.94);z-index:31;pointer-events:none;opacity:0;padding:5px 14px;border:1px solid #ff5ca466;background:linear-gradient(90deg,transparent,#07131ee6 18%,#07131ee6 82%,transparent);color:#ff5ca4;text-shadow:0 2px 10px #000;letter-spacing:3px;font-size:10px;transition:opacity .08s,transform .12s}#pbMirror.show{opacity:1;transform:translate(-50%,0) scale(1)}#pbMirror b{color:#ff5260;margin-right:8px}@media(max-height:500px){#pbMirror{top:26%}}`;document.head.appendChild(style);
 ...
 function canSpecial(){const vv=v2();return mode==='play'&&boss?.hp>0&&player?.hp>0&&boss.stun<=0&&boss.wind<=0&&boss.strike<=0&&boss.broken<=0&&!(vv?.ultimateT>0)}
