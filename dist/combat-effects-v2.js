...
(()=>{
 ...
 const EFFECTS={
  ...
  parry:{life:.27,flash:.26,glow:78,rays:22,rayLen:90,ring:.55,shake:.38,stop:.092,push:.09,color:'#ffd36b',core:'#fff0b0'},
  perfect:{life:.34,flash:.42,glow:104,rays:30,rayLen:125,ring:.78,shake:.50,stop:.118,push:.15,color:'#ffd36b',core:'#ffffff'},
 ...
 function drawPlayerParryAura(alpha){
  ...
  const phase=cl(parryAura/.42),gold=parryAuraPerfect?'#ffe08a':'#ffd36b';
  ...
  const hand=player.nodes.find(n=>n.name==='hand');if(hand){const h=project(hand.p);if(h?.z>0){const g=fctx.createRadialGradient(h.x,h.y,0,h.x,h.y,42*phase);g.addColorStop(0,'rgba(255,240,176,.94)');g.addColorStop(.28,parryAuraPerfect?'rgba(255,224,138,.58)':'rgba(255,211,107,.56)');g.addColorStop(1,'rgba(0,0,0,0)');fctx.globalAlpha=.9;fctx.fillStyle=g;fctx.fillRect(h.x-45,h.y-45,90,90)}}
  fctx.restore();
 }
 function drawBeat(beat){
  ...
  fctx.save();fctx.globalCompositeOperation='lighter';const radius=spec.glow*(.52+t*.62);const g=fctx.createRadialGradient(p2.x,p2.y,0,p2.x,p2.y,radius);g.addColorStop(0,`rgba(255,255,255,${cl(spec.flash*fade*1.75)})`);g.addColorStop(.13,(beat.kind==='parry'||beat.kind==='perfect')?'rgba(255,211,107,.72)':beat.kind==='counter'?'rgba(255,92,164,.72)':beat.kind==='playerHit'?'rgba(255,117,82,.58)':'rgba(255,82,96,.70)');g.addColorStop(.48,(beat.kind==='parry'||beat.kind==='perfect')?'rgba(255,184,74,.18)':beat.kind==='hit1'||beat.kind==='hit2'||beat.kind==='hit3'?'rgba(255,92,164,.20)':'rgba(255,82,96,.16)');g.addColorStop(1,'rgba(0,0,0,0)');fctx.fillStyle=g;fctx.fillRect(p2.x-radius,p2.y-radius,radius*2,radius*2);fctx.restore();
