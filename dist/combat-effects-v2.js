...
(()=>{
 ...
 function drawBeat(beat){
  ...
  fctx.save();fctx.globalCompositeOperation='lighter';const radius=spec.glow*(.52+t*.62);const g=fctx.createRadialGradient(p2.x,p2.y,0,p2.x,p2.y,radius);g.addColorStop(0,`rgba(255,255,255,${cl(spec.flash*fade*1.75)})`);g.addColorStop(.13,beat.kind==='counter'?'rgba(255,92,164,.72)':beat.kind==='playerHit'?'rgba(255,117,82,.58)':'rgba(255,82,96,.70)');g.addColorStop(.48,beat.kind==='hit1'||beat.kind==='hit2'||beat.kind==='hit3'?'rgba(255,92,164,.20)':'rgba(255,82,96,.16)');g.addColorStop(1,'rgba(0,0,0,0)');fctx.fillStyle=g;fctx.fillRect(p2.x-radius,p2.y-radius,radius*2,radius*2);fctx.restore();
