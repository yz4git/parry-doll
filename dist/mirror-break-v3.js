...
(()=>{
 ...
 body.mb-final-duel #mbMirrorLoadout{top:19.5%!important;min-width:260px;padding:6px 13px;background:#071019c9;border-color:#ff526048;font-size:8px;line-height:1.5;transition:opacity .1s,visibility .1s}
 ...
 body.mb-final-duel.mb-mirror-shifting #mbMirrorLoadout,body.mb-final-duel.mb-mirror-shifting #mbBreakHud,body.mb-final-duel.mb-mirror-shifting #mbZeroTrace{opacity:0!important;visibility:hidden!important}
 #mbFinalAura{position:fixed;inset:0;z-index:4;pointer-events:none;opacity:0;transition:opacity .32s;background:radial-gradient(circle at 50% 44%,rgba(255,92,164,.13),transparent 31%),linear-gradient(180deg,rgba(255,92,164,.035),transparent 35%,rgba(255,82,96,.025));mix-blend-mode:screen}
 body.mb-final-duel #mbFinalAura{opacity:.52}body.mb-final-duel.mb-final-p2 #mbFinalAura{opacity:.68;background:radial-gradient(circle at 50% 43%,rgba(255,82,96,.14),transparent 29%),radial-gradient(circle at 50% 58%,rgba(255,92,164,.07),transparent 37%)}body.mb-final-duel.mb-final-p3 #mbFinalAura{opacity:.82;background:radial-gradient(circle at 50% 43%,rgba(255,82,96,.16),transparent 24%),radial-gradient(circle at 50% 48%,rgba(255,92,164,.11),transparent 40%)}
 #mbMirrorShift{position:fixed;left:50%;top:43%;transform:translate(-50%,-50%) scale(.96);z-index:36;pointer-events:none;opacity:0;text-align:center;padding:7px 18px;border-left:1px solid #ff526088;border-right:1px solid #ff526088;background:linear-gradient(90deg,transparent,#06101bd9 19%,#06101bd9 81%,transparent);text-shadow:0 3px 14px #000;transition:opacity .12s,transform .12s;white-space:nowrap}
 #mbMirrorShift.show{opacity:1;transform:translate(-50%,-50%) scale(1)}#mbMirrorShift small{display:block;color:#b8cbc7;font-size:8px;letter-spacing:4px}#mbMirrorShift b{display:block;margin-top:3px;color:#ff5260;font-size:17px;letter-spacing:6px}#mbMirrorShift em{display:block;margin-top:3px;color:#c5d8d4;font-style:normal;font-size:8px;letter-spacing:2px}
 #mbZeroTrace{position:absolute;left:50%;bottom:max(62px,calc(env(safe-area-inset-bottom) + 55px));transform:translateX(-50%);z-index:28;pointer-events:none;opacity:0;padding:4px 9px;border:1px solid #ff526044;background:#071018a8;color:#bdd3cd;font-size:8px;letter-spacing:2px;text-shadow:0 2px 8px #000;transition:opacity .12s,transform .12s}#mbZeroTrace.show{opacity:.86}#mbZeroTrace.hot{opacity:1;transform:translateX(-50%) scale(1.06);color:#ff5260;border-color:#ff526088}
 ...
 function showShift(phase){
  ...
  const p=boss?.nodes?.find(n=>n.name==='chest')?.p||boss?.nodes?.[1]?.p;if(p){ring(p,phase===3?'#ff5260':'#ff5ca4');burst(p,phase===3?'#ff5260':'#ff5ca4',phase===3?18:10,phase===3?6:4)}
 }
 function syncFinalHud(){
  const final=level===4&&boss&&boss.hp>0&&mode==='play',phase=final?Math.max(1,v2.finalPhase||1):0;
  document.body.classList.toggle('mb-final-duel',final);document.body.classList.toggle('mb-final-p2',final&&phase===2);document.body.classList.toggle('mb-final-p3',final&&phase>=3);document.body.classList.toggle('mb-mirror-shifting',final&&s.shiftT>0);
  if(final){
 ...
 // The final duel already communicates BODY BREAK persistently. Do not add a second large center-screen BREAK toast.
 const v3Announce=announce;announce=function(text,duration){const t=String(text||'');if(level===4&&state.part?.label&&t.includes(state.part.label)&&t.includes('BREAK'))return;return v3Announce(text,duration)};
