...
(()=>{
 ...
 body.mb-final-duel #mbSynergyHud,body.mb-ruin-finisher #mbSynergyHud,body.mb-chain-destruction #mbSynergyHud{display:none!important;opacity:0!important;visibility:hidden!important}
 ...
 @media(max-height:500px){#mbSynergyHud{top:78px;right:max(18px,env(safe-area-inset-right));max-width:190px;padding-top:2px}#mbSynergyHud>b{font-size:7px}#mbSynergyHud>small,#mbSynergyHud>em,#mbSynergyProc{font-size:6px}#mbCoreChoice .mb-fusion-preview{margin-top:4px;padding-top:4px;font-size:6.5px}#mbCoreChoice .mb-fusion-current{margin-top:5px;font-size:7px}}
 `;document.head.appendChild(style);
 function ensureHud(){
  let hud=document.getElementById('mbSynergyHud');if(!hud){hud=document.createElement('div');hud.id='mbSynergyHud';document.body.appendChild(hud)}else if(hud.parentNode!==document.body)document.body.appendChild(hud);
  if(!hud.querySelector(':scope > b')||!hud.querySelector(':scope > small')||!hud.querySelector(':scope > em')||!hud.querySelector('#mbSynergyProc'))hud.innerHTML='<b></b><small></small><em></em><div id="mbSynergyProc"></div>';
  return{hud,title:hud.querySelector(':scope > b'),sub:hud.querySelector(':scope > small'),status:hud.querySelector(':scope > em'),proc:hud.querySelector('#mbSynergyProc')};
 }
 function showProc(text,color='#f0d28d'){s.lastProc=text;s.procT=.62;const ui=ensureHud();ui.proc.textContent=text;ui.proc.style.color=color;ui.proc.classList.add('show')}
 function syncHud(){
  const ui=ensureHud(),f=fusion();if(!f){ui.hud.classList.remove('show','hot');ui.title.textContent='';ui.sub.textContent='';ui.status.textContent='';ui.proc.classList.toggle('show',s.procT>0&&!!s.lastProc);return}
  ui.hud.classList.add('show');ui.title.textContent=`${f.name} ${f.lv}`;ui.title.style.color=f.color;ui.sub.textContent=f.jp;
  ...
  ui.status.textContent=status;ui.hud.classList.toggle('hot',(s.armT>0&&!!s.armed)||(f.id==='RESONANT_MIRROR'&&v6.mirrorChainT>0)||(f.id==='ZERO_TRACE'&&v3.zeroCharges>0));ui.proc.classList.toggle('show',s.procT>0&&!!s.lastProc);if(s.procT<=0){ui.proc.textContent='';ui.proc.classList.remove('show')}
 }
 ...
 const baseEnemyImpact=enemyImpact;enemyImpact=function(move=null){
  ...
  if(didPerfect&&f){if(f.id==='REFLECT_EDGE'||f.id==='RUPTURE_DRIVE')arm(f,point);else if(f.id==='ZERO_TRACE'&&(v3.zeroCharges||0)>bz)showProc('ZERO TRACE · CHARGED',f.color)}if(didParry&&f?.id==='RESONANT_MIRROR'&&preChain>0)resonantPulse(f);return out;
 };
 ...
 function forceFusion(move,call){const pool=ENEMY_MOVES[4];if(!pool?.length)return call(move);const originals=pool.slice(),saved={...state.cores},forced=originals.map(()=>clone(move));pool.splice(0,pool.length,...forced);state.cores.EDGE=state.cores.MIRROR=state.cores.PULSE=0;try{return call(move)}finally{state.cores.EDGE=saved.EDGE;state.cores.MIRROR=saved.MIRROR;state.cores.PULSE=saved.PULSE;pool.splice(0,pool.length,...originals)}}
 ...
 const priorDiag=window.parryMirrorBreakDiagnostics;window.parryMirrorBreakDiagnostics=()=>{const d=priorDiag?priorDiag():{},f=fusion(),ui=ensureHud();return{...d,v16:true,coreFusionCombat:true,coreFusion:f?.name||null,coreFusionLevel:f?.lv||0,coreFusionArmed:s.armed,coreFusionArm:+s.armT.toFixed(2),reflectEdgeHits:s.reflectHits,reflectEdgeEchoes:s.reflectEchoes,ruptureDriveHits:s.ruptureHits,resonantMirrorProcs:s.resonantProcs,zeroTraceFusionProcs:s.zeroProcs,fusionArmedAttacks:s.armedAttacks,finalFusionCalls:s.finalCalls,finalFusionAttacks:s.finalFusionAttacks,lastFinalFusion:s.lastFinalFusion,fusionChoiceEnhanced:s.choicePasses>0,lastFusionProc:s.lastProc,fusionHudParent:ui.hud.parentNode===document.body?'body':'other',fusionHudHealthy:!!(ui.title&&ui.sub&&ui.status&&ui.proc)}};
 window.parryCoreFusionDiagnostics=()=>{const f=fusion(),ui=ensureHud();return{v16:true,fusion:f?.name||null,level:f?.lv||0,armed:s.armed,arm:+s.armT.toFixed(2),reflectHits:s.reflectHits,reflectEchoes:s.reflectEchoes,ruptureHits:s.ruptureHits,resonantProcs:s.resonantProcs,zeroProcs:s.zeroProcs,finalFusionAttacks:s.finalFusionAttacks,lastFinalFusion:s.lastFinalFusion,hudHealthy:!!(ui.title&&ui.sub&&ui.status&&ui.proc),hudParent:ui.hud.parentNode===document.body?'body':'other'}};
