...
updateEnemy=function(dt){
 ...
 const bias=(FLOW_BIAS[move?.kind]||.04)*side,bodyOpen=FLOW_BODY_OPEN[level]*side;

 // During the readable wind-up, keep the torso visibly open while the actual hit aim remains committed separately.
 if(boss.wind>0){
  if(boss.wind>.32){boss.aim+=bias;boss.face=facing+bodyOpen*.62;}
  else boss.face=boss.aim+bodyOpen*.38;
  const creep=['combo','sweep','sidestep'].includes(move?.kind)?FLOW_ORBIT[level]*.50:FLOW_ORBIT[level]*.20;
  boss.vel.x+=tangent.x*creep*side;boss.vel.z+=tangent.z*creep*side;
 }
 // On the exact wind->strike transition, preserve a smaller side-on body angle instead of snapping fully square.
 if(beforeWind>0&&boss.strike>0&&beforeStrike<=0){boss.aim+=bias;boss.face=boss.aim+bodyOpen*.28;}
 if(boss.strike>0)boss.face=boss.aim+bodyOpen*.18;
 ...
 if(boss.wind<=0&&boss.strike<=0&&boss.stun<=0){
  ...
  const recovery=boss.ai>0?1.30:1;
  ...
  const blend=boss.ai>0?.44:.32;
  boss.vel.x+=(targetVel.x-boss.vel.x)*blend;boss.vel.z+=(targetVel.z-boss.vel.z)*blend;
  boss.face=facing+FLOW_BODY_OPEN[level]*flowOrbitSide*(boss.ai>0?1:.82);
 }
};
...
playerAttack=function(){
 ...
 const lateral=[.82,1.30,1.08][idx]*(player.swing?.finisher?.48:1);
 ...
 player.face=targetFace+side*(player.swing?.finisher?.08:.15);
 return true;
};

// The core lock-on still handles hit logic, but the rendered body opens 25-45 degrees while strafing.
...
Doll.prototype.physics=function(dt){
 if(this.player&&mode==='play'&&this.hp>0&&this.down<=0&&this.stun<=0){
  ...
  if(this.attack<=0&&this.parry<=0&&speed>.32){
   const moveDir=Math.atan2(this.vel.x,this.vel.z),delta=clamp(angleDelta(moveDir,toBoss),-.98,.98);
   this.face=toBoss+delta*.80;
  }else if(this.attack<=0&&this.parry<=0){
   this.face=toBoss+Math.sin(flowClock*.85)*.10;
  }
 }
 return flowPhysics.call(this,dt);
};

// Recovery from a landed hit carries tangential momentum so fighters do not reset to the same line immediately.
...
resolveSwing=function(){
 ...
 if(move&&boss.hp<before&&boss.hp>0){
  ...
  player.vel.x+=tangent.x*.52*side;player.vel.z+=tangent.z*.52*side;
  boss.vel.x-=tangent.x*.38*side;boss.vel.z-=tangent.z*.38*side;
 }
 return out;
};
