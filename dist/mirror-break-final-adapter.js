...
(()=>{
 ...
 // Keep the parry beat readable: BODY BREAK HUD already communicates exposure, so do not replace PERFECT
 // with a second center-screen message or duplicate BREAK CHANCE in the attack warning strip.
 ...
 const uiEnemyImpact=enemyImpact;enemyImpact=function(move=null){const out=uiEnemyImpact(move),d=document.getElementById('pbDanger');if(d?.textContent?.includes('BREAK CHANCE'))d.classList.remove('show');return out};
 ...
 const priorDiag=window.parryMirrorBreakDiagnostics;window.parryMirrorBreakDiagnostics=()=>{const d=priorDiag?priorDiag():{};return {...d,mirrorDominant:dominant(state.cores),finalAdapter:true,uiSimplified:true}};
})();
