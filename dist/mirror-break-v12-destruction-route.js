'use strict';
// MIRROR BREAK v12 — DESTRUCTION ROUTE: each boss KO permanently changes the architecture of the next arena.
(()=>{
 ...
 const s={history:[],lastSerial:0,routeLevel:-1,routeSource:-1,routeName:null,routeMeshes:0,routeBuilds:0};window.__mirrorBreakV12State=s;
 ...
 function build(vs,entry){const source=entry.level,routeLevel=source+1,pal=PAL[source],box=find(vs.scene,o=>o.geometry?.type==='BoxGeometry')?.geometry,ico=find(vs.scene,o=>o.geometry?.type==='IcosahedronGeometry')?.geometry,torus=find(vs.scene,o=>o.geometry?.type==='TorusGeometry')?.geometry;if(!box)return;const dark=material(vs,pal[0],.96),base=material(vs,pal[1],1),accent=material(vs,pal[2],.92,true),{side,back}=slabPath(vs,box,base,accent,entry,source),x0=side*3.6,z0=back;
  ...
  if(source===0){
   // Fallen dojo beam becomes a split gate in the dungeon: a visible open passage behind the next boss.
   ...
   // Broken cage is rebuilt as an incomplete arch, leaving a torn route through the next arena.
   ...
   // Belfry spine crosses the next skyline as one huge diagonal silhouette.
   ...
   // Final arena inherits the collapsed altar: two broken columns frame a permanent central rift.
 ...
 const baseReset=reset;reset=function(l=0){record(window.__mirrorBreakV9State?.stageHit);const prev=typeof level==='number'?level:-1,out=baseReset(l);if(l===0&&prev>=4){s.history.length=0;s.lastSerial=0;s.routeLevel=-1;s.routeSource=-1;s.routeName=null}return out};
 ...
 window.parryDestructionRouteDiagnostics=()=>({v12:true,history:s.history.map(e=>({...e})),level:s.routeLevel,source:s.routeSource,name:s.routeName,meshes:s.routeMeshes,builds:s.routeBuilds});
})();
