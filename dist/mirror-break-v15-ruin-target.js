...
(()=>{
 ...
 #mbRuinTarget{position:fixed;left:50%;top:50%;z-index:29;pointer-events:none;opacity:0;visibility:hidden;transform:translate(-50%,-50%) scale(.90);min-width:118px;padding:5px 10px;text-align:center;background:linear-gradient(90deg,transparent,#071019cf 18%,#071019cf 82%,transparent);border-left:1px solid #9fd9cf55;border-right:1px solid #9fd9cf55;text-shadow:0 2px 10px #000;white-space:nowrap}
 #mbRuinTarget.show{transform:translate(-50%,-50%) scale(1)}#mbRuinTarget.ready{filter:drop-shadow(0 0 8px #f0d38366);border-color:#f2d48e99;background:linear-gradient(90deg,transparent,#10140fe8 15%,#10140fe8 85%,transparent)}
 ...
 function paint(on,ready=false){const suppressed=v14.activeT>0||document.body.classList.contains('mb-chain-destruction')||document.body.classList.contains('mb-ruin-finisher');const shown=!!on&&!suppressed;hud.classList.toggle('show',shown);hud.classList.toggle('ready',shown&&ready);hud.style.opacity=shown?(ready?'1':'.72'):'0';hud.style.visibility=shown?'visible':'hidden';hud.style.transform=shown?'translate(-50%,-50%) scale(1)':'translate(-50%,-50%) scale(.90)'}
 function hide(){s.visible=false;s.ready=false;s.lastReady=false;paint(false,false)}
 ...
 const priorDiag=window.parryMirrorBreakDiagnostics;window.parryMirrorBreakDiagnostics=()=>{const d=priorDiag?priorDiag():{},cs=getComputedStyle(hud);return{...d,v15:true,ruinTargeting:true,ruinTargetVisible:s.visible,ruinExecutionReady:s.ready,ruinTargetSource:s.source,ruinTargetRoute:s.route,ruinTargetThreshold:+s.threshold.toFixed(1),ruinTarget:s.target?{...s.target}:null,ruinTargetConsumed:s.consumed,ruinTargetUpdates:s.updates,ruinTargetClass:hud.className,ruinTargetOpacity:+(Number(cs.opacity)||0).toFixed(2),ruinTargetVisibility:cs.visibility}};
 window.parryRuinTargetDiagnostics=()=>{const cs=getComputedStyle(hud);return{v15:true,visible:s.visible,ready:s.ready,source:s.source,route:s.route,threshold:+s.threshold.toFixed(1),target:s.target?{...s.target}:null,consumed:s.consumed,updates:s.updates,className:hud.className,opacity:+(Number(cs.opacity)||0).toFixed(2),visibility:cs.visibility}};
 update();
})();
