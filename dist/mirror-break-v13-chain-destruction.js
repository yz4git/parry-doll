...
(()=>{
 ...
 function collapseLoop(c){if(!c||s.collapse!==c)return;animateCollapse(performance.now());if(s.collapse===c&&!c.settled)requestAnimationFrame(()=>collapseLoop(c))}
 function prepareCollapse(source,now){const vs=s.scene,meshes=vs?.__mbDestructionRoute?.meshes||[];if(!meshes.length){s.collapse=null;return}const take=Math.min(source===0?4:source===1?6:source===2?7:8,Math.max(0,meshes.length-7));const chosen=meshes.slice(Math.max(7,meshes.length-take)),c={key:key(),source,start:now,settled:false,items:chosen.map((m,i)=>({m,i,px:m.position.x,py:m.position.y,pz:m.position.z,rx:m.rotation.x,ry:m.rotation.y,rz:m.rotation.z}))};s.collapse=c;requestAnimationFrame(()=>collapseLoop(c))}
 function detachMirrorShift(){if(s.mirrorMount)return;const el=document.getElementById('mbMirrorShift');if(!el?.parentNode)return;const parent=el.parentNode,next=el.nextSibling;s.mirrorMount={el,parent,next};el.classList.remove('show');parent.removeChild(el)}
 function restoreMirrorShift(){const m=s.mirrorMount;if(!m)return;const{el,parent,next}=m;s.mirrorMount=null;if(el.isConnected)return;if(next?.parentNode===parent)parent.insertBefore(el,next);else parent.appendChild(el)}
 function prioritizeChain(){const token=++s.uiToken;document.body.classList.add('mb-chain-destruction');detachMirrorShift();setTimeout(()=>{if(s.uiToken===token){document.body.classList.remove('mb-chain-destruction');restoreMirrorShift()}},1600)}
 function trigger(source,launch,now){const e=entry(),k=key(e);if(!e||s.consumed.has(k))return false;s.consumed.add(k);s.triggers++;const p=contactPoint(),route=NAMES[source],q=targetFor(source),away=V(p.x-q.x,0,p.z-q.z),dir=mag(away)>.01?norm(away):V(source%2?1:-1,0,.25),damage=8+source*3,secondary=add(mul(dir,13+source*2.5),V(0,4.2+source*.65,0));
  prioritizeChain();prepareCollapse(source,now);groundImpact(V(p.x,.06,p.z),1.75+source*.42);burst(V(p.x,.18,p.z),COLORS[source],20+source*5,5.2+source*.75);ring(V(p.x,.08,p.z),COLORS[source]);shake=Math.max(shake,.22+source*.035);announce(`連 鎖 崩 壊 — ${route}`,1.05);
 ...
 const baseReset=reset;reset=function(l=0){const prev=typeof level==='number'?level:-1,out=baseReset(l);s.launch=null;s.collapse=null;s.scene=null;s.uiToken++;document.body.classList.remove('mb-chain-destruction');restoreMirrorShift();if(l===0&&prev>=4){s.triggers=0;s.history.length=0;s.consumed.clear();s.last=null}s.checks=0;return out};
 const priorDiag=window.parryMirrorBreakDiagnostics;window.parryMirrorBreakDiagnostics=()=>{animateCollapse(performance.now());const d=priorDiag?priorDiag():{};const mirror=document.getElementById('mbMirrorShift');return{...d,v13:true,chainDestruction:true,chainTriggers:s.triggers,chainHistory:s.history.map(e=>({...e})),chainLast:s.last?{...s.last}:null,chainArmed:!!s.launch,chainPower:s.launch?.power||0,chainChecks:s.checks,chainCollapseActive:!!s.collapse,chainCollapseSettled:!!s.collapse?.settled,chainConsumed:[...s.consumed],chainUiPriority:document.body.classList.contains('mb-chain-destruction'),chainMirrorDetached:!!s.mirrorMount,chainMirrorDisplay:mirror?getComputedStyle(mirror).display:null}};
 window.parryChainDestructionDiagnostics=()=>{animateCollapse(performance.now());const mirror=document.getElementById('mbMirrorShift');return{v13:true,triggers:s.triggers,last:s.last?{...s.last}:null,armed:!!s.launch,power:s.launch?.power||0,checks:s.checks,collapse:!!s.collapse,settled:!!s.collapse?.settled,consumed:[...s.consumed],uiPriority:document.body.classList.contains('mb-chain-destruction'),mirrorDetached:!!s.mirrorMount,mirrorDisplay:mirror?getComputedStyle(mirror).display:null}};
})();

// V14 loads only after v13 has installed its chain state and wrappers.
(()=>{if(window.__parryMirrorBreakV14Queued)return;window.__parryMirrorBreakV14Queued=true;const src=document.currentScript?.src||'',q=src.includes('?')?'?'+src.split('?').slice(1).join('?'):'',v14=document.createElement('script');v14.src='./mirror-break-v14-ruin-finisher.js'+q;document.body.appendChild(v14)})();
