...
(()=>{
 ...
 const s={launch:null,triggers:0,history:[],consumed:new Set(),applying:false,collapse:null,scene:null,last:null,checks:0,uiToken:0};window.__mirrorBreakV13State=s;
 const uiStyle=document.createElement('style');uiStyle.textContent='body.mb-chain-destruction #mbMirrorShift{opacity:0!important;visibility:hidden!important;transform:translate(-50%,-50%) scale(.96)!important}';document.head.appendChild(uiStyle);
 ...
 function prioritizeChain(){const token=++s.uiToken;document.body.classList.add('mb-chain-destruction');setTimeout(()=>{if(s.uiToken===token)document.body.classList.remove('mb-chain-destruction')},1600)}
 ...
 const baseReset=reset;reset=function(l=0){const prev=typeof level==='number'?level:-1,out=baseReset(l);s.launch=null;s.collapse=null;s.scene=null;s.uiToken++;document.body.classList.remove('mb-chain-destruction');if(l===0&&prev>=4){s.triggers=0;s.history.length=0;s.consumed.clear();s.last=null}s.checks=0;return out};
 const priorDiag=window.parryMirrorBreakDiagnostics;window.parryMirrorBreakDiagnostics=()=>{animateCollapse(performance.now());const d=priorDiag?priorDiag():{};return{...d,v13:true,chainDestruction:true,chainTriggers:s.triggers,chainHistory:s.history.map(e=>({...e})),chainLast:s.last?{...s.last}:null,chainArmed:!!s.launch,chainPower:s.launch?.power||0,chainChecks:s.checks,chainCollapseActive:!!s.collapse,chainCollapseSettled:!!s.collapse?.settled,chainConsumed:[...s.consumed],chainUiPriority:document.body.classList.contains('mb-chain-destruction')}};
 window.parryChainDestructionDiagnostics=()=>{animateCollapse(performance.now());return{v13:true,triggers:s.triggers,last:s.last?{...s.last}:null,armed:!!s.launch,power:s.launch?.power||0,checks:s.checks,collapse:!!s.collapse,settled:!!s.collapse?.settled,consumed:[...s.consumed],uiPriority:document.body.classList.contains('mb-chain-destruction')}};
})();
